...
import { blockTypeToBlockName, useToolbarState } from '../../lexical/context/ToolbarContext';
...
import { EmbedConfigs } from '../../lexical/plugins/AutoEmbedPlugin';
import { INSERT_COLLAPSIBLE_COMMAND } from '../../lexical/plugins/CollapsiblePlugin';
import { InsertEquationDialog } from '../../lexical/plugins/EquationsPlugin';
import {
  INSERT_IMAGE_COMMAND,
  InsertImageDialog,
  InsertImagePayload,
} from '../../lexical/plugins/ImagesPlugin';
import { InsertInlineImageDialog } from '../../lexical/plugins/InlineImagePlugin';
import InsertLayoutDialog from '../../lexical/plugins/LayoutPlugin/InsertLayoutDialog';
import { INSERT_PAGE_BREAK } from '../../lexical/plugins/PageBreakPlugin';
import { InsertPollDialog } from '../../lexical/plugins/PollPlugin';
import { SHORTCUTS } from '../../lexical/plugins/ShortcutsPlugin/shortcuts';
import { InsertStrapiImageDialog } from '../../lexical/plugins/StrapiImagePlugin';
import { InsertTableDialog } from '../../lexical/plugins/TablePlugin';
import {
  clearFormatting,
  formatBulletList,
  formatCheckList,
...
function ElementFormatDropdown({
  ...
  const hasIndentOptions = indent || outdent;
  ...
  return (
    ...
    >
      {showAlignmentOptions && leftAlign && (
        <DropDownItem
          onClick={() => {
      ...
      {showAlignmentOptions && centerAlign && (
        <DropDownItem
          onClick={() => {
      ...
      {showAlignmentOptions && rightAlign && (
        <DropDownItem
          onClick={() => {
      ...
      {showAlignmentOptions && justifyAlign && (
        <DropDownItem
          onClick={() => {
      ...
      {showAlignmentOptions && startAlign && (
        <DropDownItem
          onClick={() => {
...
export default function CustomizableToolbar({
  editor,
  activeEditor,
  setActiveEditor,
  setIsLinkEditMode,
  ...
  const insertLinkCallback = useCallback(() => {
  ...
  const hasAnyTextFormat = bold || italic || underline || inlineCode;
  ...
  const filteredEmbedConfigs = EmbedConfigs.filter((config) => {
    if (config.type === 'youtube-video') return youtubeEmbed;
    if (config.type === 'tweet') return twitterEmbed;
    if (config.type === 'figma') return figmaEmbed;
    return false;
  });

  const hasAnyHistoryButton = undo || redo;
  const hasAnyLinkOrMediaButton = insertLink || insertStrapiImage;
  const hasAnyColorPicker = fontColor || backgroundColor;
  const hasAnyFontControl = fontFamily || fontSize;
  ...
  return (
    <div className="customizable-toolbar toolbar">
      ...
      {isStrapiImageDialogOpen && insertStrapiImage && (
      ...
      {codeBlock && codeLanguageSelector && toolbarState.blockType === 'code' ? (
        <DropDown
          disabled={!isEditable}
        ...
        <>
          ...
          {inlineCode && canViewerSeeInsertCodeButton && (
            <button
              disabled={!isEditable}
              onClick={() => {
          ...
          {(indent || outdent) &&
            (leftAlign || centerAlign || rightAlign || justifyAlign || startAlign || endAlign) && (
              <Divider />
            )}
          {(indent || outdent) && (
          ...
          {canViewerSeeInsertDropdown && hasAnyInsertOption && (
            <>
              {(insertTable || insertLink || insertStrapiImage) && <Divider />}
              <DropDown
                disabled={!isEditable}
                buttonClassName="toolbar-item spaced"
                buttonLabel={formatMessage({
              ...
              >
                ...
                {filteredEmbedConfigs.map((embedConfig) => (
                  <DropDownItem
